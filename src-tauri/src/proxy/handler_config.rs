...
/// Pi `/pi/v1/messages` 解析配置（Anthropic Messages 协议）
...
/// Pi `/pi/v1/chat/completions` 解析配置（OpenAI Chat Completions 协议）。
///
/// 使用 fresh-input 解析变体：OpenAI 的 `prompt_tokens` 是 cache-inclusive，
/// 而 Pi 的 session 行与 proxy 行统一按 FRESH 口径入库（"pi" 不在
/// CACHE_INCLUSIVE_APP_TYPES），跨源去重指纹按存储值精确比较，必须同口径。
pub const PI_CHAT_PARSER_CONFIG: UsageParserConfig = UsageParserConfig {

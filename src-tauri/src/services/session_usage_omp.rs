//! Omp (`omp` CLI) 会话用量追踪
//!
//! 从 `~/.omp/agent/sessions/<cwd>/<ts>_<uuid>/*.jsonl`（v3，与 Pi 同格式）中的
//! assistant 消息 + compaction + branch_summary 事件提取 token 用量，写入
//! `proxy_request_logs`，覆盖无代理直连态下的统计。每个 `.jsonl` 视为一个会话
//! （主会话与子代理会话文件一并计入）。
//!
//! ## 数据流
//! ```text
...
//!   ├─ type=compaction,           entry.usage?     → 自动摘要用量（无 model）
//!   └─ type=branch_summary,       entry.usage?     → 分支摘要用量（无 model）
//!        → 费用计算 → proxy_request_logs（data_source=omp_session）
//! ```
//!
//! ## 事件口径（与 Pi session v3 一致）
//! - **assistant 消息**（`type == "message"` 且 `message.role == "assistant"`）
//!   字段：`message.model`、`message.usage.{input,output,cacheRead,cacheWrite}`、
//!   条目级 `timestamp`（RFC3339）、条目级 `id`（会话内唯一）。
//! - **compaction**（`type == "compaction"`）携带可选 `.usage`，无 `.model`；
//!   用合成 `_omp_summary` 标签写入。
//! - **branch_summary**（`type == "branch_summary"`）携带可选 `.usage`，同上。
//! - `usage.input` 是新鲜输入 → `input_token_semantics = FRESH`。
//! - 幂等键 `omp_session:{session_id}:{entry_id}`：条目 id 在会话内唯一且
//!   append-only，重扫/重启都稳定。
//! - 代理接管去重走统一指纹守卫（`should_skip_session_insert`）：/omp 路由
...
struct OmpUsageEvent {
    /// 会话内唯一条目 id（参与 request_id）
    entry_id: String,
    /// epoch 秒（由条目时间戳换算）
    created_at: i64,
    /// 真实 model id 或合成标签（compaction/branch_summary → `_omp_summary`）
    model: String,
    input_tokens: u32,
    output_tokens: u32,
...
/// 同步 Omp 使用数据（从 session jsonl 会话日志）
pub fn sync_omp_usage(db: &Database) -> Result<SessionSyncResult, AppError> {
    ...
    for file_path in &files {
        match sync_single_session_file(db, file_path) {
            ...
            Err(e) => {
                let msg = format!("Omp 会话文件解析失败 {}: {e}", file_path.display());
                log::warn!("[OMP-SYNC] {msg}");
...
fn collect_session_files() -> Vec<PathBuf> {
    ...
    for root in crate::session_manager::providers::omp::session_roots() {
...
fn sync_single_session_file(
    ...
    let (session_id, events) = parse_omp_usage_events(&content, file_path);
    ...
    for event in &events {
        ...
        let request_id = format!("omp_session:{session_id}:{}", event.entry_id);

        match insert_omp_session_entry(db, &request_id, event, &session_id) {
            Ok(true) => result.imported += 1,
            Ok(false) => result.skipped += 1,
            Err(e) => {
                log::warn!("[OMP-SYNC] 插入失败 ({request_id}): {e}");
                result.skipped += 1;
            }
        }
    }
...
/// 缺失时回落文件名（如 `PluginScout.jsonl` → `PluginScout`）。
fn parse_omp_usage_events(content: &str, file_path: &Path) -> (String, Vec<OmpUsageEvent>) {
...
/// 这两类条目都没有 `.model`，统一使用合成标签 `_omp_summary`——价格表
/// 查不到时 cost=0，但 token 计数仍然进入使用统计。
fn parse_summary_event(record: &serde_json::Value) -> Option<OmpUsageEvent> {
    let entry_id = record
        .get("id")
        .and_then(|v| v.as_str())
        .filter(|s| !s.is_empty())?;
...
fn insert_omp_session_entry(
    ...
    let (input_cost, output_cost, cache_read_cost, cache_creation_cost, total_cost) =
        match find_model_pricing(&conn, &event.model) {
            Some(pricing) => {
                let cost =
                    CostCalculator::calculate_for_app("omp", &usage, &pricing, Decimal::from(1));
                (
    ...
    let inserted_rows = conn
        .execute(
            ...
            rusqlite::params![
                ...
                session_id,
                Some("omp_session"), // provider_type
                1i64,                  // is_streaming
                "1.0",                 // cost_multiplier
                created_at,
                "omp_session", // data_source
                INPUT_TOKEN_SEMANTICS_FRESH,
            ],
        )
        .map_err(|e| AppError::Database(format!("插入 Omp 会话日志失败: {e}")))?;
...
mod tests {
    ...
    fn parse_extracts_assistant_usage_and_skips_others() {
        ...
        let (session_id, events) = parse_omp_usage_events(&content, Path::new("/tmp/x.jsonl"));
    ...
    /// compaction (`type=compaction`) 与 branch_summary (`type=branch_summary`)
    /// 都可能携带 `.usage`（无 `.model`）。钉死这两类事件被识别、用合成标签
    /// `_omp_summary` 并按 entry_id 去重写入 `proxy_request_logs`。
    #[test]
    fn parse_extracts_compaction_and_branch_summary_usage() {
        ...
        let (_session_id, events) = parse_omp_usage_events(&content, Path::new("/tmp/x.jsonl"));
        assert_eq!(
            events.len(),
            2,
        ...
        assert_eq!(a.model, "_omp_summary");
        ...
        assert_eq!(b.model, "_omp_summary");
        assert_eq!(b.input_tokens, 4096);
        assert_eq!(b.output_tokens, 256);
    }

    #[test]
    fn parse_falls_back_to_filename_stem_and_message_ts() {
        let content = format!(
            "{}\n",
        ...
        let (session_id, events) =
            parse_omp_usage_events(&content, Path::new("/tmp/sess-dir/PluginScout.jsonl"));
        assert_eq!(session_id, "PluginScout");
        assert_eq!(events.len(), 1);
    ...
    fn sync_imports_usage_idempotently_and_skips_zero() -> Result<(), AppError> {
        let dir = tempdir().unwrap();
        let bucket = dir.path().join("--tmp-project--").join("sess-dir");
        fs::create_dir_all(&bucket).unwrap();
        let file = bucket.join("session.jsonl");
        {
        ...
        let row: (String, String, i64, i64) = conn.query_row(
            "SELECT request_id, app_type, input_tokens, input_token_semantics FROM proxy_request_logs WHERE data_source = 'omp_session' ORDER BY created_at LIMIT 1",
            [],
            |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?, row.get(3)?)),
        )?;
        assert_eq!(row.0, "omp_session:sess-abc:666c87cd");
        assert_eq!(row.1, "omp");
        assert_eq!(row.2, 100);
    ...
    fn insert_skips_when_matching_proxy_log_exists() -> Result<(), AppError> {
        ...
        let inserted =
            insert_omp_session_entry(&db, "omp_session:sess-abc:666c87cd", &event, "sess-abc")?;
        assert!(
            !inserted,

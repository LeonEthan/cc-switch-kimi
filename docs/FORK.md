...
| Sessions | Session Manager scans `~/.omp/agent/sessions` (mixed two/three-level layout: main sessions `<cwd>/<ts>_<uuid>.jsonl`, subagent sessions `<cwd>/<ts>_<uuid>/*.jsonl`, Pi v3 JSONL; read-only except explicit delete); resume via `omp --resume <id>` |

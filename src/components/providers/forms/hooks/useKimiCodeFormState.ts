...
export function useKimiCodeFormState({
  ...
  getSettingsConfig,
}: UseKimiCodeFormStateParams) {
  const { data: providersData } = useProvidersQuery("kimicode");
  ...
  const [models, setModels] = useState<KimiCodeModel[]>(() => {
    if (appId !== "kimicode") return [];
    return parseField(initialData, "models", KIMICODE_DEFAULT_CONFIG.models);
  });
  ...
  const writeConfig = useCallback(
    (patch: Partial<KimiCodeProviderSettingsConfig>) => {
  ...
  const resetState = useCallback(
    (config?: Partial<KimiCodeProviderSettingsConfig>) => {
      const next = { ...KIMICODE_DEFAULT_CONFIG, ...config };
